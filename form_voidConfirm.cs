...
{
    public partial class form_voidConfirm : Form
    {
        ...
        form_voidOrder voidOrderForm;

        public form_voidConfirm(form_voidOrder voidOrder)
        {
            InitializeComponent();
            sql_connect = new SqlConnection(db_connect.DBConnection());

            voidOrderForm = voidOrder;
        }
        ...
        private void btn_OK_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                sql_connect.Open();
                sql_command = new SqlCommand("SELECT * FROM tbl_users WHERE username = @username AND role LIKE 'administrator'", sql_connect);
                ...
                if (hasRows == true)
                {
                    if (tb_voidCode.Text == voidCode)
                    {
                        ...
                        if (voidOrderForm.cb_action.Text == "Return to Inventory")
                        {
                            returnToInventory();
                            updateQuantity();
                        }
                        ...
                        this.Dispose();
                        voidOrderForm.Dispose();
                        voidOrderForm.reloadSales();
                    }
                    else
                    {
        ...
        {
            double total = double.Parse(voidOrderForm.tb_price.Text) * double.Parse(voidOrderForm.tb_cancelQuantity.Text);
            ...
            sql_command.Parameters.AddWithValue("@transactionNo", voidOrderForm.tb_transactionNo.Text);
            sql_command.Parameters.AddWithValue("@productID", voidOrderForm.tb_productID.Text);
            sql_command.Parameters.AddWithValue("@price", double.Parse(voidOrderForm.tb_price.Text));
            sql_command.Parameters.AddWithValue("@quantity", int.Parse(voidOrderForm.tb_cancelQuantity.Text));
            ...
            sql_command.Parameters.AddWithValue("@cancelledBy", voidOrderForm.tb_voidBy.Text);
            sql_command.Parameters.AddWithValue("@action", voidOrderForm.cb_action.Text);
            sql_command.Parameters.AddWithValue("@remarks", voidOrderForm.tb_remarks.Text);
        ...
        {
            sql_connect.Open();
            sql_command = new SqlCommand("UPDATE tbl_products SET quantity = quantity + " + int.Parse(voidOrderForm.tb_cancelQuantity.Text) + " WHERE productID = '" + voidOrderForm.tb_productID.Text + "'", sql_connect);
            sql_command.ExecuteNonQuery();
            sql_connect.Close();
        }
        ...
        {
            sql_connect.Open();
            sql_command = new SqlCommand("UPDATE tbl_transaction SET quantity = quantity - " + int.Parse(voidOrderForm.tb_cancelQuantity.Text) + " WHERE transactionID LIKE '" + voidOrderForm.transactionID.Text + "'", sql_connect);
            sql_command.ExecuteNonQuery();
            sql_connect.Close();
        }
